import { useState, useRef, useEffect } from 'react';

interface LeverageSelectorProps {
  value: number;
  onChange: (value: number);
}

export default function LeverageSelector({ value, onChange }: LeverageSelectorProps) {
  const leverageRef = useRef<HTMLDivElement>(null);
  const isLeverageDragging = useRef(false);
  const leverageOptions = [5, 10, 25, 50, 75, 100, 125];
  const [selectedLeverageIndex, setSelectedLeverageIndex] = useState(1); // Default to 10x

  useEffect(() => {
    if (onChange && typeof onChange === 'function') {
      onChange(leverageOptions[selectedLeverageIndex]);
    }
  }, [selectedLeverageIndex]);

  const updateLeverageFromPosition = (clientX: number) => {
    if (!leverageRef.current) return;
    
    const rect = leverageRef.current.getBoundingClientRect();
    const x = clientX - rect.left;
    const percentage = Math.max(0, Math.min(100, (x / rect.width) * 100));
    
    const newIndex = Math.round((percentage / 100) * (leverageOptions.length - 1));
    setSelectedLeverageIndex(newIndex);
  };

  const handleLeverageMouseDown = (e: React.MouseEvent) => {
    isLeverageDragging.current = true;
    updateLeverageFromPosition(e.clientX);
  };

  const handleLeverageMouseMove = (e: MouseEvent) => {
    if (isLeverageDragging.current) {
      updateLeverageFromPosition(e.clientX);
    }
  };

  const handleLeverageMouseUp = () => {
    isLeverageDragging.current = false;
  };

  const handleLeverageTouchStart = (e: React.TouchEvent) => {
    isLeverageDragging.current = true;
    updateLeverageFromPosition(e.touches[0].clientX);
  };

  const handleLeverageTouchMove = (e: TouchEvent) => {
    if (isLeverageDragging.current && e.touches[0]) {
      updateLeverageFromPosition(e.touches[0].clientX);
    }
  };

  const handleLeverageTouchEnd = () => {
    isLeverageDragging.current = false;
  };

  useEffect(() => {
    document.addEventListener('mousemove', handleLeverageMouseMove);
    document.addEventListener('mouseup', handleLeverageMouseUp);
    document.addEventListener('touchmove', handleLeverageTouchMove);
    document.addEventListener('touchend', handleLeverageTouchEnd);

    return () => {
      document.removeEventListener('mousemove', handleLeverageMouseMove);
      document.removeEventListener('mouseup', handleLeverageMouseUp);
      document.removeEventListener('touchmove', handleLeverageTouchMove);
      document.removeEventListener('touchend', handleLeverageTouchEnd);
    };
  }, []);

  const leverageProgress = (selectedLeverageIndex / (leverageOptions.length - 1)) * 100;

  return (
    <div className="trading-panel-leverage-section">
      <div className="trading-panel-leverage-label">
        <span className="label-text" style={{ opacity: 0.72 }}>Leverage</span>
        <span className="label-value">{leverageOptions[selectedLeverageIndex]}x</span>
      </div>
      
      <div className="relative w-full">
        <div 
          className="relative h-[24px] w-full cursor-pointer"
          ref={leverageRef}
          onMouseDown={handleLeverageMouseDown}
          onTouchStart={handleLeverageTouchStart}
        >
          <div 
            className="absolute left-0 right-0 top-0 h-[24px] rounded-[36px] px-[3px] py-[4px]"
            style={{
              background: 'linear-gradient(180deg, #2E3138 0%, #1A1D23 100%)',
              border: '3px solid #52566a',
              boxShadow: '0px 4px 4px 0px rgba(0,0,0,0.25), inset 0px 1px 8px 0px black',
            }}
          >
            <div className="flex items-start justify-between h-full px-0 py-[2px] w-full">
              {leverageOptions.slice(0, -1).map((_, i) => (
                <div key={i} className="bg-[rgba(255,255,255,0.1)] h-full w-[2px]" />
              ))}
            </div>
          </div>

          <div 
            className="absolute left-[4px] top-1/2 -translate-y-1/2 h-[16px] rounded-[28px] transition-all duration-200 ease-out"
            style={{
              width: `calc(${leverageProgress}% - ${leverageProgress * 0.04}px + 16px)`,
              maxWidth: 'calc(100% - 4px)',
              minWidth: '24px',
              background: 'linear-gradient(180deg, #2F7AEC 0%, #245BB8 100%)',
              boxShadow: '0px 1px 6px 0px rgba(47,122,236,0.4), inset 0px 1px 1px 0px rgba(0,0,0,0.25)',
              zIndex: 1,
            }}
          >
            <div 
              className="absolute right-[-4px] top-1/2 -translate-y-1/2 w-[36px] h-[36px] z-[3] pointer-events-none"
              style={{
                background: 'conic-gradient(from 25deg at 50% 50%, #EBEBEB 8.526539951562881deg, #A6A6A6 31.273336708545685deg, #EBEBEB 60.399945974349976deg, #B8B8B8 74.53422725200653deg, #EDEDED 84.85412299633026deg, #ABABAB 93.26865792274475deg, #F0F0F0 130.0709366798401deg, #D1D1D1 156.95856928825378deg, #B0B0B0 175.30829071998596deg, #EBEBEB 184.46789503097534deg, #DEDEDE 190.88155031204224deg, #EBEBEB 208.86048316955566deg, #C7C7C7 227.78544187545776deg, #C4C4C4 256.4226794242859deg, #ADADAD 264.3653440475464deg, #BDBDBD 269.2746877670288deg, #E5E5E5 276.247980594635deg, #ABABAB 348.82035970687866deg, #C2C2C2 355.37188053131104deg, #CCC 360deg)',
                border: '2.5px solid #E7E7E7',
                borderRadius: '50%',
                boxShadow: '1px 4px 6px rgba(0, 0, 0, 0.36), 0 1px 0 rgba(0, 0, 0, 0.12)',
              }}
            >
              <div
                className="absolute inset-[2px] rounded-full"
                style={{
                  background: 'radial-gradient(circle at 35% 35%, rgba(255, 255, 255, 0.9) 0%, rgba(255, 255, 255, 0.3) 30%, transparent 60%)',
                  pointerEvents: 'none',
                }}
              />
            </div>
          </div>
        </div>

        <div className="relative w-full mt-[4px]">
          <div className="flex items-center justify-between w-full px-[3px]">
            {leverageOptions.map((option, index) => (
              <div 
                key={option}
                className="flex flex-col items-center justify-center cursor-pointer"
                onClick={() => setSelectedLeverageIndex(index)}
              >
                <p className="relative shrink-0 font-['IBM_Plex_Sans_Condensed:SemiBold',sans-serif] text-[16px] leading-normal text-black">
                  {option}x
                </p>
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  );
}
